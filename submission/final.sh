...
TX_INPUTS=
TX_INPUTS=$(jq -nc --arg txid "$UTXO_TXID" --argjson vout "$UTXO_VOUT_INDEX" '[{txid:$txid,vout:$vout,sequence:1}]')
...
TX_OUTPUTS=
TX_OUTPUTS=$(jq -nc --arg payment_address "$PAYMENT_ADDRESS" --arg payment_btc "$PAYMENT_BTC" --arg change_address "$CHANGE_ADDRESS" --arg change_btc "$CHANGE_BTC" '{($payment_address): ($payment_btc | tonumber), ($change_address): ($change_btc | tonumber)}')
...
VERIFY_PAYMENT=
VERIFY_PAYMENT=$(echo "$DECODED_TX" | jq -r --arg address "$PAYMENT_ADDRESS" '.vout[] | select(.scriptPubKey.address == $address) | .value' | awk '{printf "%.8f", $1}')
check_cmd "Payment verification" "VERIFY_PAYMENT" "$VERIFY_PAYMENT"

VERIFY_CHANGE=
VERIFY_CHANGE=$(echo "$DECODED_TX" | jq -r --arg address "$CHANGE_ADDRESS" '.vout[] | select(.scriptPubKey.address == $address) | .value' | awk '{printf "%.8f", $1}')
...
# WRITE YOUR SOLUTION BELOW:
CHANGE_OUTPUT_INDEX=
CHANGE_OUTPUT_INDEX=$(echo "$DECODED_TX" | jq -r --arg address "$CHANGE_ADDRESS" '.vout[] | select(.scriptPubKey.address == $address) | .n')
...
# WRITE YOUR SOLUTION BELOW:
CHILD_INPUTS=
CHILD_INPUTS=$(jq -nc --arg txid "$PARENT_TXID" --argjson vout "$CHANGE_OUTPUT_INDEX" '[{txid:$txid,vout:$vout,sequence:1}]')
...
# STUDENT TASK: Create the outputs JSON structure
CHILD_OUTPUTS=
CHILD_OUTPUTS=$(jq -nc --arg child_recipient "$CHILD_RECIPIENT" --arg child_send_btc "$CHILD_SEND_BTC" '{($child_recipient): ($child_send_btc | tonumber)}')
...
# WRITE YOUR SOLUTION BELOW:
TIMELOCK_INPUTS=
TIMELOCK_INPUTS=$(jq -nc --arg txid "$SECONDARY_TXID" '[{txid:$txid,vout:1,sequence:10}]')
...
# STUDENT TASK: Create the outputs JSON structure
TIMELOCK_OUTPUTS=
TIMELOCK_OUTPUTS=$(jq -nc --arg timelock_address "$TIMELOCK_ADDRESS" --arg timelock_btc "$TIMELOCK_BTC" '{($timelock_address): ($timelock_btc | tonumber)}')
